...
LAST_STEP_DECREASING_EPS = 2e5   # epsilon will decrease from 1 to 0.1 until this step
...
parser.add_argument('--total_frames', type=float, default=1e6, metavar='N', help='number of frames for training')
...
parser.add_argument('--step_size_testing', type=int, default=1000, help='step size of testing')
        ...
        if random.random() > eps:
            ...
            with torch.no_grad():
                ...
                for k in range(args.num_model - 1, -1,-1):  # loop from the newest to the oldest model, in case later needed for recency-weighting avg
                    action_values += self.qnetwork_targets[self.qnetwork_targets_idx[k]](state)
                # action_values = self.qnetwork_local(state)
            self.qnetwork_local.train()
            return np.argmax(action_values.cpu().data.numpy())
        else:
...
def evaluate(args, agent, eps):
    ...
    for i in range(args.step_size_testing):  # evaluate per step_size_testing
        value_estimate, action_from_net = agent.take_value_estimate(state)
        # Epsilon-greedy action selection
        if random.random() > eps:
    ...
    return mean_score, value_est/n_steps
...
def store_json(scores, value_ests):
    ...
    with open('log/recent_' + env_name.lower()+'_k'+str(args.num_model)+"_seed"+str(args.seed)+".json", 'w') as outfile:
        json.dump(log, outfile)


def dqn(agent, total_frames, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.995): #paper use eps_end = 0.1
    """Deep Q-Learning.
